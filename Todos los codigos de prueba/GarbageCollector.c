...
int main()
{
	...
	remove_reference(block2); // Quitar una referencia más al mismo bloque 1-> 0 ---free 300 , memoria utilizada = 450, disp= 550
...
int add_reference(int block)
{
	...
	for (int j = 0; j < cantidad_bloques; j++)
	{
		printf("Estoy en el for de refference\n");
		if (*(identificador + j) == block)
		{
	...
	printf("La posición coincidente es: %d\n", position);
	printf("El contador de referencias antes de sumar es: %d\n", *(cont_referencias + position));
...
int remove_reference(int block)
{
	printf("----- estoy en REMOVE reference y el id del bloque es %d\n", block);
	...
	for (int j = 0; j < cantidad_bloques; j++)
	{
		printf("Estoy en el for de REMOVE\n");
		if (*(identificador + j) == block)
		{
			printf("--- estoy en el if del for\n");
			position = j;
	...
	printf("La posición coincidente es: %d\n", position);
	printf("El contador de referencias antes de RESTAR es: %d\n", *(cont_referencias + position));
